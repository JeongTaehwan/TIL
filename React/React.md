...
> 이런 식으로 코드를 작성하여 사용할 수 있다. `import React, { useState } from 'react'` 이런식으로 `useState` 를 불러와 사용할 수 있다.

---

**useRef** 🤡
> `useRef` 는 특정 `DOM` 을 직접 선택할 때 사용된다. 조회나 수정이 가능한 변수를 관리할 때 사용된다.
...
> `useCallback` 은 `useMemo` 와 비슷하다. `useMemo` 는 특정결과 값을 재사용하지만, `useCallback` 은 특정함수를 새로 만들지 않고 재사용할때 사용한다.
