...
- 위의 코드에서 보다시피 `get` 이라는 함수를 불러와 `'이름을 출력합니다'` 라는 문장을 `return` 하게 된다.
...
- prototype은 객체와 객체를 연결 시켜주고, 불필요한 메모리를 줄여준다.
